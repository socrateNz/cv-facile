    ...
    <div className="bg-white text-gray-900 font-sans relative">
      <div className="grid grid-cols-3 min-h-[680px]">
        <div className="p-8 text-white flex flex-col justify-between" style={{ backgroundColor: primaryColor }}>
          <div>
            <h1 className="text-3xl font-bold">{cv.fullName || "Nom complet"}</h1>
            <p className="text-lg opacity-90 mt-3">{cv.title || "Titre professionnel"}</p>
          </div>
            ...
            <div className="mt-8 pt-8 border-t border-white/30">
              ...
              <div className="space-y-2">
                {cv.skills.slice(0, 6).map((skill, idx) => (
                  <p key={`${skill}-${idx}`} className="font-semibold text-sm">
                    {skill}
                  </p>
                ))}
              </div>
              ...
              <div className="space-y-6">
                {cv.experiences.map((exp, idx) => (
                  <div key={`${exp.company}-${idx}`}>
                    ...
                    <p className="text-gray-700 mt-2 leading-relaxed text-sm">{exp.summary}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                {cv.education.map((edu, idx) => (
                  <div key={`${edu.school}-${idx}`} className="pb-4 border-b border-gray-200 last:border-b-0">
                    <h3 className="font-bold text-gray-900">{edu.degree || "Diplôme"}</h3>
                    <p className="text-sm text-gray-600">{edu.school || "Établissement"}</p>
